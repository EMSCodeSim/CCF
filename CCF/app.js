...
const UI = {
  ...
  btnBpmUp: $("btnBpmUp"),
  metroBpm: $("metroBpm"),
  // coach bars (these live in the "metroVisual" card)
  breathFill: $("breathFill"),
  breathIcon: $("breathIcon"),
  breathMain: $("breathMain"),
  breathSub: $("breathSub"),
  pulseFill: $("pulseFill"),
  pulseMain: $("pulseMain"),
  pulseSub: $("pulseSub"),
...
function updateMetroUI(){
  ...
  // The "metroVisual" card now shows coaching bars; keep this hint short.
  UI.metroHint.textContent = enabled
    ? (state.metroOn ? "Metronome on (clicks during CPR)" : "Turn on metronome for clicks")
    : "Metronome available during CPR";
}

// ---- coaching bars ----
function updateCoachBars(){
  if (!UI.breathFill || !UI.pulseFill) return;

  // Breath cycle: 30 compressions at current BPM
  const cycleMs = Math.max(3000, Math.round((30 * 60000) / Math.max(30, state.bpm))); // min 3s safety
  const remainingBreathMs = Math.max(0, cycleMs - state.breathCprMs);
  const breathFrac = state.breathsDue ? 0 : (remainingBreathMs / cycleMs);
  UI.breathFill.style.width = `${Math.round(breathFrac * 100)}%`;

  if (state.breathsDue){
    UI.breathIcon.textContent = "🫁";
    UI.breathMain.textContent = "Give 2 breaths";
    UI.breathSub.textContent = "Then resume CPR";
  } else {
    UI.breathIcon.textContent = "🫁";
    UI.breathMain.textContent = `Next breaths in ${Math.ceil(remainingBreathMs/1000)}s (~30 compressions)`;
    UI.breathSub.textContent = "Breaths (30:2)";
  }

  // Pulse check: every 2 minutes of total session time (CPR + hands-off)
  const pulseCycleMs = 120000;
  const totalMs = state.compMs + state.offMs;
  const pulseMod = totalMs % pulseCycleMs;
  const remainingPulseMs = pulseCycleMs - pulseMod;
  const pulseFrac = remainingPulseMs / pulseCycleMs;
  UI.pulseFill.style.width = `${Math.round(pulseFrac * 100)}%`;
  UI.pulseMain.textContent = `Next pulse check in ${fmt(remainingPulseMs)}`;
  UI.pulseSub.textContent = "Pulse Check";
}
...
function tick(){
  ...
  if (state.mode === "cpr"){
    state.compMs += dt;
    // Breath coaching: only counts compressions time
    if (!state.breathsDue){
      const cycleMs = 1800000 / Math.max(60, state.bpm); // 30 compressions @ bpm
...
function startCPR(){
  ...
  // If breaths were due, the user likely delivered them during the pause.
  // Reset breath coach when compressions resume.
  if (state.breathsDue){
